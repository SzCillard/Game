...
class MCTSNode:
    ...
    def select(self):
        ...
        if len(self.children) == 0:
            return self
        return max(self.children, key=lambda c: c.wins / (c.visits + 1) + math.sqrt(2 * math.log(self.visits + 1) / (c.visits + 1)))
...
def mcts_search(start_state, iterations=100):
    ...
    return max(root.children, key=lambda c: c.visits).state if root.children else start_state
